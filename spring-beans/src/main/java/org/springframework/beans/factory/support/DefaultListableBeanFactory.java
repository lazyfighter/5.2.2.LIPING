...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory implements ConfigurableListableBeanFactory, BeanDefinitionRegistry, Serializable {
	...
	/**
	 * 所有beanDefinition的名称，按照注册时间排序的
	 */
	private volatile List<String> beanDefinitionNames = new ArrayList<>(256);

	/**
	 * 所有手动注册的beanDefinition的名称，按照注册时间进行排序
	 */
	private volatile Set<String> manualSingletonNames = new LinkedHashSet<>(16);

	/**
	 * 缓存所有冻结的beanDefinition的名称
	 */
	@Nullable
	private volatile String[] frozenBeanDefinitionNames;

	/**
	 * 是否缓存冻结beanDefinition
	 */
	...
	/**
	 * 设置是否允许同名的beanName进行beanDefinition覆盖
	 */
	...
	/**
	 * 是否允许提交加载bean对应的class 即使该bean是lazy_load
	 */
		...
		for (String beanName : this.beanDefinitionNames) {
			...
			if (!isAlias(beanName)) {
				try {
					...
					// 非抽象类 && (允许提前初始化 || (有具体的class || 不是懒加载 || 允许提前classLoading)) &&
	...
	private <T> NamedBeanHolder<T> resolveNamedBean(ResolvableType requiredType, @Nullable Object[] args, boolean nonUniqueAsNull) throws BeansException {
		...
		if (candidateNames.length == 1) {
			...
			// 如果有多个候选beanName的时候，判断获取有@Promary注解
			String candidateName = determinePrimaryCandidate(candidates, requiredType.toClass());
			if (candidateName == null) {
				// 如果有多个候选beanName的时候，同时都没有标注@Promary注解， 则按照javax规则获取标注@Priority的
				candidateName = determineHighestPriorityCandidate(candidates, requiredType.toClass());
			}
			if (candidateName != null) {
				Object beanInstance = candidates.get(candidateName);
				// 该bean还没有进行实例化 || 该bean不是单例的或者有构造参数的， 需要按照参数进行实例化的
